...
__global__ void trilinear_fw_kernel(
    ...
    torch::PackedTensorAccessor32<scalar_t, 2, torch::RestrictPtrTraits> feat_interp)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(feats.scalar_type(), "trilinear_fw_cu",
                               ([&]
                                { trilinear_fw_kernel<scalar_t><<<blocks, threads>>>(
                                      ...
                                      feat_interp.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>()); }));
...
__global__ void trilinear_bw_kernel(
    const torch::PackedTensorAccessor32<scalar_t, 2, torch::RestrictPtrTraits> dL_dfeat_interp,
    const torch::PackedTensorAccessor32<scalar_t, 3, torch::RestrictPtrTraits> feats,
    const torch::PackedTensorAccessor32<scalar_t, 2, torch::RestrictPtrTraits> points,
    torch::PackedTensorAccessor32<scalar_t, 3, torch::RestrictPtrTraits> dL_dfeats)
{
    const int n = blockIdx.x * blockDim.x + threadIdx.x;
    const int f = blockIdx.y * blockDim.y + threadIdx.y;
...
{
    const int N = feats.size(0), F = feats.size(2);
    printf("trilinear_bw_cu: N = %d, F = %d\n", N, F);
    ...
    AT_DISPATCH_FLOATING_TYPES(feats.scalar_type(), "trilinear_bw_cu",
                               ([&]
                                { trilinear_bw_kernel<scalar_t><<<blocks, threads>>>(
                                      dL_dfeat_interp.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
                                      feats.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(),
                                      points.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
                                      dL_dfeats.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>()); }));
